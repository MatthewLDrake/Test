    ...
    public partial class Form1 : Form
    {
        ...
        private void AddPlayersToLists(int i)
        {
            ...
            for(int j = 0; j < players.Count; j++)
            {
                if (!players[j].scout)
                {
                    lists[i - 1].Rows.Add(players[j].ToString(), "?", "?", "?", "?", "?", "?", "?", "?", "?", "?", "?", "?");
                }
                else
                {
                    lists[i-1].Rows.Add(players[j].ToString(), players[j].GetLayup(), players[j].GetDunk(), players[j].GetJumpshot(), players[j].GetThreePoint(), players[j].GetPass(), players[j].GetShotContest(), players[j].GetDefenseIQ(), players[j].GetJumping(), players[j].GetSeperation(), players[j].GetDurability(), players[j].GetStamina(), players[j].GetPotential());
                }
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 1; i <= 5; i++)
            {
                ...
                for (int j = 0; j < players.Count; j++)
                {
                    if (!players[j].scout)
                    {
                        str += players[j].ToString() +"," + players[j].GetPosition() + ",?,?,?,?,?,?,?,?,?,?,?,?," + players[j].GetAge() + "\n";
                    }
                    else
                    {
                        str += players[j].ToString()+ "," + players[j].GetPosition() + "," + players[j].GetLayup()+ "," + players[j].GetDunk()+ "," + players[j].GetJumpshot()+ "," + players[j].GetThreePoint()+ "," + players[j].GetPass()+ "," + players[j].GetShotContest()+ "," + players[j].GetDefenseIQ()+ "," + players[j].GetJumping()+ "," + players[j].GetSeperation()+ "," + players[j].GetDurability()+ "," + players[j].GetStamina()+ "," + players[j].GetPotential() +"," + players[j].GetAge() + "\n";
                    }
                }
            }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 1; i <= 5; i++)
            {
                ...
                for (int j = 0; j < players.Count; j++)
                {
                    ...
                    {
                        str += players[j].ToString() + "," + players[j].GetPosition() + "," + players[j].GetLayup().Replace(" -> ",",") + "," + players[j].GetDunk().Replace(" -> ", ",") + "," + players[j].GetJumpshot().Replace(" -> ", ",") + "," + players[j].GetThreePoint().Replace(" -> ", ",") + "," + players[j].GetPass().Replace(" -> ", ",") + "," + players[j].GetShotContest().Replace(" -> ", ",") + "," + players[j].GetDefenseIQ().Replace(" -> ", ",") + "," + players[j].GetJumping().Replace(" -> ", ",") + "," + players[j].GetSeperation().Replace(" -> ", ",") + "," + players[j].GetDurability().Replace(" -> ", ",") + "," + players[j].GetStamina().Replace(" -> ", ",") + "," + players[j].GetPotential()+ "," + players[j].GetAge() + "\n";
                    }
                }
            }
